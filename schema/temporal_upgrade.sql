-- Idempotent schema advances applied on top of temporal.sql to reach the
-- version recorded in temporalSchemaVersion. These run against both a
-- freshly-applied schema and a database initialized by an earlier release,
-- so every statement must be safe to re-run.

-- v1.19: separate archetype ID space (current_chasm_executions)
